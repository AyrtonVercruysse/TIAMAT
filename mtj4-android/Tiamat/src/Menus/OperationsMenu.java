...
public class OperationsMenu extends Menu{
	...
	public OperationsMenu(AbstractMTApplication mtApplication, String name) {
		super(mtApplication, name);
	}
	
	private MTListCell createListCell(final AbstractMTApplication mtApplication, final String label, final AST.Node node, IFont font, float cellWidth, float cellHeight, final MTColor cellFillColor, final MTColor cellPressedFillColor){
		...
		cell.addGestureListener(TapProcessor.class, new IGestureEventListener() {
			public boolean processGestureEvent(MTGestureEvent ge) {
				...
				case TapEvent.BUTTON_CLICKED:
					...
					System.out.println(parent.getChildren()[0]);
					...
					System.out.println(parent.getChildren()[0]);
					//Het setten van de current:
					//kijken naar de current, zijn parent vragen met getParent()
					//de parent zijn get children doen
					//kinder overlopen met de == operator naar de current
					//dit kind setten
					cell.setFillColor(cellFillColor);
					if (StartTiamat.selected == null){
						//If null toevoegen aan de main loop.
					}else{
						System.out.println(StartTiamat.selected);
					}
				}
		...
		public void Make(AbstractMTApplication mtApplication, String name){
			...
			for(int i=0; i< StartTiamat.operations.size(); i++){
				Tiamat.Templates template = StartTiamat.operations.get(i);
