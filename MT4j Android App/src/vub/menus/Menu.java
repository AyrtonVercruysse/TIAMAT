...
public abstract class Menu extends AbstractScene {
	...
	public void makeListLabel(MTAndroidApplication mtApplication, IFont font,
			String label) {
	...
	public void makeCell(MTAndroidApplication mtApplication, float cellWidth,
			float cellHeight, MTColor cellFillColor) {
	...
	public void makeMapMenu(MTAndroidApplication mtApplication, Boolean out) {
		mapMenu = new MTRoundRectangle(mtApplication, 0, 0, 0, 250, 1140, 20,
				20);
		mapMenu.setFillColor(new MTColor(35, 35, 35, 180));
		mapMenu.setStrokeColor(new MTColor(35, 35, 35, 180));
		if (out) {
			mapMenu.setPositionGlobal(new Vector3D(
					mtApplication.width / 2f + 170, mtApplication.height / 2f));

		} else {
			mapMenu.setPositionGlobal(new Vector3D(mtApplication.width / 2f,
				...
				new IGestureEventListener() {
					public boolean processGestureEvent(MTGestureEvent ge) {
						...
						if (te.isTapped()) {
							...
							vub.tiamat.Tiamat.beginMenu.Make(mtApplication,
									"BeginMenu", true);
						}
						return true;
					}
	...
	abstract public void Make(MTAndroidApplication mtApplication, String name,
			Boolean out);
